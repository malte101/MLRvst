...
float mapGrainEnvelopePhase(float normPos, float skew)
{
    const float clampedPos = juce::jlimit(0.0f, 1.0f, normPos);
    const float clampedSkew = juce::jlimit(0.05f, 0.95f, skew);

    if (clampedPos <= clampedSkew)
        return 0.5f * (clampedPos / clampedSkew);

    return 0.5f + (0.5f * ((clampedPos - clampedSkew) / (1.0f - clampedSkew)));
}

float shapeGrainEnvelopeWindow(float baseWindow, float curve)
{
    const float clampedWindow = juce::jlimit(0.0f, 1.0f, baseWindow);
    const float clampedCurve = juce::jlimit(0.35f, 3.0f, curve);
...
float quantizeSpeedRatioMusical(float unit, bool bipolar)
{
    // Share one musical ratio palette between Mod Speed and Playhead Speed controls.
    // Bipolar mode centers on 1x so the lane can move slower/faster around neutral.
    ...
    constexpr int kCenterIndex = 8; // 1x
    int idx = kCenterIndex;
    if (clamped < 0.5f)
    {
        idx = juce::jlimit(
            0,
            kCenterIndex,
            static_cast<int>(std::round(juce::jmap(clamped, 0.0f, 0.5f, 0.0f, static_cast<float>(kCenterIndex)))));
    }
    else
    {
        idx = juce::jlimit(
            kCenterIndex,
            kMaxIndex,
            static_cast<int>(std::round(juce::jmap(clamped, 0.5f, 1.0f, static_cast<float>(kCenterIndex), static_cast<float>(kMaxIndex)))));
    }
...
double retriggerDivisionFromAmount(float amount01)
{
    ...
    // 2.0=1/2, 1.0=1/4, 0.5=1/8, 0.25=1/16, 0.125=1/32, 0.0625=1/64, 0.03125=1/128
    ...
    if (v < 0.9375f) return 0.0625; // 1/64
    return 0.03125;                 // 1/128
}
...
void Crossfader::startFade(bool fadeIn, int numSamples, bool forceRestartFromEdge)
{
    ...
    if (!forceRestartFromEdge && active)
    {
        // Resume: getNextValue derives the gain purely from the ramp ratio,
        // so continuing from the current gain requires mapping that gain back
        // onto the new ramp's phase (fadeIn gain = sin(ratio*pi/2), fadeOut
        // gain = cos(ratio*pi/2)). Without this the "resume" restarted at the
        // edge and stepped the output in one sample.
        constexpr float halfPi = 1.57079632679f;
        const float gain = juce::jlimit(0.0f, 1.0f, currentGain.load());
        const float startRatio = fadeIn ? (std::asin(gain) / halfPi)
                                        : (std::acos(gain) / halfPi);
        samplesRemaining = juce::jlimit(
            1,
            numSamples,
            numSamples - static_cast<int>(std::round(startRatio * static_cast<float>(numSamples))));
        active = true;
        return;
    }
...
void QuantizationClock::scheduleTrigger(int stripIndex,
                                        int column,
                                        double ppq,
                                        ...
                                        int64_t sampleStartSample)
{
    ...
    t.column = column;
    t.sampleSliceId = sampleSliceId;
    t.sampleStartSample = sampleStartSample;
...
void QuantizationClock::queueExactTrigger(const QuantisedTrigger& trigger)
{
    const juce::SpinLock::ScopedLockType lock(pendingTriggersLock);
    auto insertPos = std::upper_bound(
        pendingTriggers.begin(),
        pendingTriggers.end(),
        trigger.targetSample,
        [](int64_t sample, const QuantisedTrigger& pendingTrigger)
        {
            return sample < pendingTrigger.targetSample;
        });
    auto stampedTrigger = trigger;
    stampTrigger(stampedTrigger);
    pendingTriggers.insert(insertPos, stampedTrigger);
}
...
void QuantizationClock::seedTimelineState(int64_t samplePosition, double ppqPosition)
{
    const juce::SpinLock::ScopedLockType lock(pendingTriggersLock);
    currentSample.store(juce::jmax<int64_t>(0, samplePosition), std::memory_order_release);
    currentPPQ = std::isfinite(ppqPosition) ? ppqPosition : 0.0;
}
...
void QuantizationClock::clearPendingTriggersForStrip(int stripIndex, bool bumpGeneration)
{
    const juce::SpinLock::ScopedLockType lock(pendingTriggersLock);
    ...
    // Invalidate events already popped into a block's local list (a release
    // arriving after the pop must still cancel the press it targets).
    // Clear-on-fire dedup passes bumpGeneration=false: it only needs the
    // queue erase, and bumping there would wrongly drop legitimate later
    // same-block events (replayed re-presses, macro/mod retriggers).
    if (bumpGeneration && stripIndex >= 0 && stripIndex < kMaxCancelStrips)
        stripCancelGenerations[static_cast<size_t>(stripIndex)]
...
void PatternRecorder::startRecording(double currentBeat)
{
    ...
    autoStartPlaybackAfterRecording.store(true, std::memory_order_release);
...
void PatternRecorder::startOverdub(double currentBeat)
{
    if (events.empty())
    {
    ...
    const int length = juce::jmax(1, lengthInBeats.load(std::memory_order_acquire));
    double startBeat = std::ceil(currentBeat);
    ...
    if (keepPlaying && std::isfinite(anchor) && anchor >= 0.0)
    {
        const double loopBeats = static_cast<double>(length);
        const double relativeBeat = juce::jmax(0.0, currentBeat - anchor);
        startBeat = anchor + (std::ceil(relativeBeat / loopBeats) * loopBeats);
    }

    recordingStartBeat.store(startBeat, std::memory_order_release);
    recordingEndBeat.store(startBeat + static_cast<double>(length), std::memory_order_release);
    recordingOverdub.store(true, std::memory_order_release);
    autoStartPlaybackAfterRecording.store(false, std::memory_order_release);
    recording.store(true, std::memory_order_release);
    ...
    DBG("Pattern overdub scheduled: start=" << startBeat
        << ", end=" << (startBeat + static_cast<double>(length))
        << ", length=" << length << " beats");
}
...
void PatternRecorder::extendOverdubRecording()
{
    ...
    recordingEndBeat.store(endBeat + static_cast<double>(length), std::memory_order_release);
}
...
double PatternRecorder::getPlaybackProgressForBeat(double currentBeat) const
{
    if (!playing.load(std::memory_order_acquire) || !std::isfinite(currentBeat))
        return -1.0;
    ...
    if (currentBeat <= anchor)
        return 0.0;

    double localBeat = std::fmod(currentBeat - anchor, static_cast<double>(length));
    if (localBeat < 0.0)
        localBeat += static_cast<double>(length);

    return juce::jlimit(0.0, 1.0, localBeat / static_cast<double>(length));
}
...
bool PatternRecorder::updateRecording(double currentBeat)
{
    ...
    if (currentBeat >= endBeat)
    {
        ...
        const bool shouldAutoStartPlayback = autoStartPlaybackAfterRecording.load(std::memory_order_acquire);
        stopRecording();
...
void PatternRecorder::recordEvent(int strip,
                                  int column,
                                  bool noteOn,
                                  double currentBeat,
                                  int sampleSliceId,
                                  int64_t sampleStartSample)
{
    ...
    {
        const double loopBeats = static_cast<double>(juce::jmax(1, lengthInBeats.load(std::memory_order_acquire)));
        double wrappedTime = std::fmod(currentBeat - startBeat, loopBeats);
        if (wrappedTime < 0.0)
            wrappedTime += loopBeats;
        event.time = wrappedTime;
    }
    else
    {
    ...
    DBG("Event recorded: strip=" << strip
        << ", col=" << column
...
void EnhancedAudioStrip::prepareToPlay(double sampleRate, int maxBlockSize)
{
    ...
    const int combMaxDelaySamples = juce::jmax(8, static_cast<int>(std::ceil(sampleRate / 20.0)) + 8);
    filterCombBuffer.setSize(2, combMaxDelaySamples, false, true, true);
    resetCombFilterState();
...
void EnhancedAudioStrip::loadSample(const juce::AudioBuffer<float>& buffer, double sourceRate)
{
    ...
    if (playMode == PlayMode::Step)
    {
        ...
        stepSamplerLoadedVersion = pitchSourceVersion.load(std::memory_order_acquire);
...
void EnhancedAudioStrip::adoptPreparedSample(juce::AudioBuffer<float>& preparedSampleBuffer,
                                             ...
                                             double preparedLoopTempoMatchHostTempo,
                                             double preparedLoopTempoMatchBeatsForLoop,
                                             double preparedLoopTempoMatchSourceSampleRate,
                                             TimeStretchBackend preparedLoopTempoMatchBackend)
{
    ...
    {
        DBG("WARNING: Attempting to adopt empty prepared buffer into strip");
        return;
    }
...
#if MLRVST_ENABLE_BUNGEE
    const bool hasPreparedTempoMatchCache = preparedLoopTempoMatchBuffer != nullptr
        && preparedLoopTempoMatchBackend == TimeStretchBackend::Bungee
        && preparedLoopTempoMatchBuffer->getNumSamples() > 0
        && preparedLoopTempoMatchBuffer->getNumChannels() > 0
        && preparedLoopTempoMatchHostTempo > 0.0
        && preparedLoopTempoMatchBeatsForLoop > 0.0
        && preparedLoopTempoMatchSourceSampleRate > 0.0
        && sampleBuffer.getNumSamples() > 0
        && std::abs(preparedLoopTempoMatchSourceSampleRate - sourceSampleRate) <= 1.0e-4;

    if (hasPreparedTempoMatchCache)
    {
        loopTempoMatchCacheBuffer = std::move(*preparedLoopTempoMatchBuffer);
        loopTempoMatchCacheValid = loopTempoMatchCacheBuffer.getNumSamples() > 0;
        loopTempoMatchCacheHostTempo = preparedLoopTempoMatchHostTempo;
        loopTempoMatchCacheBeatsForLoop = preparedLoopTempoMatchBeatsForLoop;
        loopTempoMatchCacheSourceSampleRate = preparedLoopTempoMatchSourceSampleRate;
        loopTempoMatchCacheSourceSamples = sampleBuffer.getNumSamples();
        loopTempoMatchCacheBackend = preparedLoopTempoMatchBackend;
        loopTempoMatchCachePositionScale = static_cast<double>(loopTempoMatchCacheBuffer.getNumSamples())
            / static_cast<double>(juce::jmax(1, sampleBuffer.getNumSamples()));
    }
    else
    {
        invalidateLoopTempoMatchCache();
...
#else
    juce::ignoreUnused(preparedLoopTempoMatchBuffer,
                       preparedLoopTempoMatchHostTempo,
                       preparedLoopTempoMatchBeatsForLoop,
                       preparedLoopTempoMatchSourceSampleRate,
                       preparedLoopTempoMatchBackend);
#endif
}
...
void EnhancedAudioStrip::spawnGrainVoice(double centerSamplePos, float sizeMs, float density, float spread, float pitchOffsetSemitones, double playbackStepBase)
{
    ...
    if (voiceIndex < 0)
    {
        ...
        for (int i = 0; i < voiceCount; ++i)
        {
            ...
            // Prefer voices already in their quiet envelope edges: exit ramp
            // first (ending anyway), then just-born (still near-silent), then
            // closest-to-end — residual steals cut the least audible grain.
            double score = (progress * 2.0) + (static_cast<double>(candidate.ageSamples) * 1.0e-6);
            if (progress > 0.92)
                score += 8.0;
...
void EnhancedAudioStrip::renderGrainAtSample(float& outL, float& outR, double centerSamplePos, double effectiveSpeed, int64_t globalSample)
{
    ...
    const juce::AudioBuffer<float>* grainSourceBuffer = &sampleBuffer;
    double grainPlaybackPositionScale = 1.0;
#if MLRVST_ENABLE_BUNGEE
    if (loopTempoMatchCacheValid
        && getTempoMatchBackend() == TimeStretchBackend::Bungee
        && loopTempoMatchCacheBuffer.getNumSamples() > 0)
    {
        grainSourceBuffer = &loopTempoMatchCacheBuffer;
        grainPlaybackPositionScale = juce::jmax(1.0e-9, loopTempoMatchCachePositionScale);
    }
#endif
    const auto mapGrainReadPosition = [&](double logicalPosition)
    {
        return logicalPosition * grainPlaybackPositionScale;
    };
    ...
    const double neutralSourceReadPos = mapGrainReadPosition(neutralReadPos);
    const double neutralStep = 1.0;
    const float neutralSampleL = grainResampler.getSample(*grainSourceBuffer, 0, neutralSourceReadPos, neutralStep);
    const float neutralSampleR = (grainSourceBuffer->getNumChannels() > 1)
        ? grainResampler.getSample(*grainSourceBuffer, 1, neutralSourceReadPos, neutralStep)
        : neutralSampleL;
    const float entryDrySampleL = resampler.getSample(*grainSourceBuffer, 0, neutralSourceReadPos, neutralStep);
    const float entryDrySampleR = (grainSourceBuffer->getNumChannels() > 1)
        ? resampler.getSample(*grainSourceBuffer, 1, neutralSourceReadPos, neutralStep)
        : entryDrySampleL;
    ...
    int activeGrainVoicesNow = 0;
    for (const auto& grainVoiceState : grainVoices)
        if (grainVoiceState.active)
            ++activeGrainVoicesNow;

    if (!entryIdentityScheduler)
    {
        // Soft pool ceiling: taper spawn demand as the voice pool approaches
        // capacity so sustained max density/emitter settings stop stealing
        // sounding grains (the continuous-crackle source). Brief bursts may
        // still steal — those pick the least audible voice.
        const double poolHeadroom = juce::jlimit(0.0, 1.0,
            ((0.9 * static_cast<double>(kMaxGrainVoices)) - static_cast<double>(activeGrainVoicesNow))
                / (0.35 * static_cast<double>(kMaxGrainVoices)));
        spawnRate *= poolHeadroom;

        // Leaving the neutral passthrough with an empty pool: spawn NOW.
        // The accumulator froze below 1 while neutral (rate scaled by a ~0
        // blend), which previously left up to ~1.3s of silence after the
        // first gesture or param nudge.
        if (activeGrainVoicesNow == 0
            && granularBlend > 0.05f
    ...
    for (auto& voice : grainVoices)
    {
        ...
        const float envelopeNormPos = mapGrainEnvelopePhase(normPos, voice.envelopeSkew);
        const int windowIdx = juce::jlimit(0, static_cast<int>(grainWindow.size()) - 1,
            static_cast<int>(std::round(envelopeNormPos * static_cast<float>(grainWindow.size() - 1))));
        const float window = shapeGrainEnvelopeWindow(grainWindow[static_cast<size_t>(windowIdx)], voice.envelopeCurve);
        float env = 1.0f;
        if (neutralBlend < 0.9999f)
        {
            ...
            switch (grainQuality)
            {
                ...
                {
                    const float tri = 1.0f - std::abs((envelopeNormPos * 2.0f) - 1.0f);
        ...
        if (fade > 1.0e-4f)
        {
            ...
            const float edgeDistance = juce::jmin(envelopeNormPos, 1.0f - envelopeNormPos);
            ...
            const float centerTri = juce::jlimit(0.0f, 1.0f, 1.0f - std::abs((envelopeNormPos * 2.0f) - 1.0f));
...
#if MLRVST_ENABLE_BUNGEE
void EnhancedAudioStrip::invalidateLoopTempoMatchCache()
{
    loopTempoMatchCacheValid = false;
    loopTempoMatchCacheBuffer.setSize(0, 0, false, false, true);
    loopTempoMatchCacheHostTempo = -1.0;
    loopTempoMatchCacheBeatsForLoop = -1.0;
    loopTempoMatchCacheSourceSampleRate = -1.0;
    loopTempoMatchCacheSourceSamples = -1;
    loopTempoMatchCacheBackend = TimeStretchBackend::Resample;
    loopTempoMatchCachePositionScale = 1.0;
}

bool EnhancedAudioStrip::rebuildLoopTempoMatchCache(double hostTempo, double beatsForLoop)
{
    const auto backend = getTempoMatchBackend();
    ...
    {
        invalidateLoopTempoMatchCache();
        tempoMatchRenderRequested.store(0, std::memory_order_release);
        return false;
    }

    // Quantize the tempo key so a host tempo ramp does not thrash rebuilds
    // (target length error at 0.1 bpm is sub-millisecond over a bar; actual
    // timing comes from the PPQ mapping, not the cache length).
    const double quantizedTempo = std::round(hostTempo * 10.0) / 10.0;
    const int sourceSamples = sampleBuffer.getNumSamples();
    const double targetDurationSeconds = (beatsForLoop * 60.0) / juce::jmax(1.0e-3, quantizedTempo);
    ...
    const int targetFrames = juce::jmax(
        1, static_cast<int>(std::llround(targetDurationSeconds * sourceSampleRate)));
    if (targetFrames == sourceSamples)
    {
        invalidateLoopTempoMatchCache();
        tempoMatchRenderRequested.store(0, std::memory_order_release);
        return false;
    ...
    // AUDIO THREAD: never render here. Blend out of the stale cache, post a
    // background render request, and play the dry (timing-correct) path
    // until the worker installs the new cache.
    if (loopTempoMatchCacheValid)
    {
        armTriggerOutputBlendLocked(
            juce::jmax(16, static_cast<int>(currentSampleRate * 0.002)),
            true);
        invalidateLoopTempoMatchCache();
    }

    const bool keyChanged =
        std::abs(tempoMatchRequestTempo.load(std::memory_order_acquire) - quantizedTempo) > 1.0e-9
        || std::abs(tempoMatchRequestBeats.load(std::memory_order_acquire) - beatsForLoop) > 1.0e-9
        || tempoMatchRenderRequested.load(std::memory_order_acquire) == 0;
    if (keyChanged)
    {
        tempoMatchRequestTempo.store(quantizedTempo, std::memory_order_release);
        tempoMatchRequestBeats.store(beatsForLoop, std::memory_order_release);
        tempoMatchRequestSerial.fetch_add(1, std::memory_order_acq_rel);
    }
    tempoMatchRenderRequested.store(1, std::memory_order_release);
    return false;
}

bool EnhancedAudioStrip::renderLoopTempoMatchCacheOffline()
{
    struct InFlightGuard
    {
        std::atomic<int>& flag;
        ~InFlightGuard() { flag.store(0, std::memory_order_release); }
    } inFlightGuard{ tempoMatchRenderInFlight };

    juce::AudioBuffer<float> sourceCopy;
    double sourceRate = 0.0;
    int sourceSamples = 0;
    TimeStretchBackend backend{};
    double quantizedTempo = 0.0;
    double beatsForLoop = 0.0;
    uint32_t requestSerial = 0;
    uint64_t sourceVersion = 0;
    {
        juce::ScopedLock lock(bufferLock);
        if (tempoMatchRenderRequested.load(std::memory_order_acquire) == 0)
            return false;
        backend = getTempoMatchBackend();
        quantizedTempo = tempoMatchRequestTempo.load(std::memory_order_acquire);
        beatsForLoop = tempoMatchRequestBeats.load(std::memory_order_acquire);
        requestSerial = tempoMatchRequestSerial.load(std::memory_order_acquire);
        if (backend != TimeStretchBackend::Bungee
            || sampleBuffer.getNumSamples() <= 0
            || sampleBuffer.getNumChannels() <= 0
            || !(sourceSampleRate > 0.0)
        ...
        sourceCopy.makeCopyOf(sampleBuffer);
        sourceRate = sourceSampleRate;
        sourceSamples = sampleBuffer.getNumSamples();
        sourceVersion = pitchSourceVersion.load(std::memory_order_acquire);
    }

    const double targetDurationSeconds = (beatsForLoop * 60.0) / quantizedTempo;
    const int targetFrames = juce::jmax(
        1, static_cast<int>(std::llround(targetDurationSeconds * sourceRate)));
    if (targetFrames == sourceSamples)
    {
        juce::ScopedLock lock(bufferLock);
        tempoMatchRenderRequested.store(0, std::memory_order_release);
    ...
    // Heavy stretch runs WITHOUT the lock, on a copy.
    juce::AudioBuffer<float> stretchedBuffer;
    const bool rendered = renderTimeStretchedBuffer(sourceCopy,
                                                    sourceRate,
                                                    targetFrames,
                                                    0.0f,
                                                    backend,
    ...
    // Drop stale results: key or source changed while rendering (the request
    // flag stays set so the poll reschedules with the new key).
    if (requestSerial != tempoMatchRequestSerial.load(std::memory_order_acquire)
        || sourceVersion != pitchSourceVersion.load(std::memory_order_acquire)
        || sampleBuffer.getNumSamples() != sourceSamples)
    {
        return false;
    }

    loopTempoMatchCacheBuffer = std::move(stretchedBuffer);
    loopTempoMatchCacheValid = loopTempoMatchCacheBuffer.getNumSamples() > 0;
    loopTempoMatchCacheHostTempo = quantizedTempo;
    loopTempoMatchCacheBeatsForLoop = beatsForLoop;
    loopTempoMatchCacheSourceSampleRate = sourceRate;
    loopTempoMatchCacheSourceSamples = sourceSamples;
    loopTempoMatchCacheBackend = backend;
    loopTempoMatchCachePositionScale = static_cast<double>(loopTempoMatchCacheBuffer.getNumSamples())
        / static_cast<double>(juce::jmax(1, sourceSamples));
    startPitchCacheOutputBlendLocked();
    tempoMatchRenderRequested.store(0, std::memory_order_release);
    return loopTempoMatchCacheValid;
}
#endif
                                                     ...
                                                     int loopCols)
{
    ...
    {
        // Transient analysis is far too heavy for the audio thread; the
        // offline render job rebuilds the map before rendering.
        if (soundTouchSwingCacheValid)
        {
            armTriggerOutputBlendLocked(
                juce::jmax(16, static_cast<int>(currentSampleRate * 0.002)), true);
            invalidateSoundTouchSwingCache();
        }
        // Bump the serial only on the request's 0->1 transition: bumping every
        // block would defeat the scheduler's serial-stability gate and the
        // render job would never run (livelock).
        if (swingCacheRenderRequested.load(std::memory_order_acquire) == 0)
            swingCacheRequestSerial.fetch_add(1, std::memory_order_acq_rel);
        swingCacheRenderRequested.store(1, std::memory_order_release);
        return false;
    }
    ...
    // AUDIO THREAD: never render here. Blend out of the stale cache, post a
    // background render request, and play the dry path until the worker
    ...
    const bool keyChanged =
        std::abs(swingCacheRequestLoopStartSamples.load(std::memory_order_acquire) - loopStartSamples) > 1.0e-3
        || std::abs(swingCacheRequestLoopLength.load(std::memory_order_acquire) - loopLength) > 1.0e-3
        || std::abs(swingCacheRequestBeats.load(std::memory_order_acquire) - beatsForLoop) > 1.0e-9
        || swingCacheRequestLoopCols.load(std::memory_order_acquire) != loopColsClamped
        || swingCacheRequestLoopStartCol.load(std::memory_order_acquire) != loopStartCol
        || swingCacheRenderRequested.load(std::memory_order_acquire) == 0;
    if (keyChanged)
    {
        swingCacheRequestLoopStartSamples.store(loopStartSamples, std::memory_order_release);
        swingCacheRequestLoopLength.store(loopLength, std::memory_order_release);
        swingCacheRequestBeats.store(beatsForLoop, std::memory_order_release);
        swingCacheRequestLoopCols.store(loopColsClamped, std::memory_order_release);
        swingCacheRequestLoopStartCol.store(loopStartCol, std::memory_order_release);
        swingCacheRequestSerial.fetch_add(1, std::memory_order_acq_rel);
    }
    swingCacheRenderRequested.store(1, std::memory_order_release);
    return false;
...
bool EnhancedAudioStrip::renderSoundTouchSwingCacheOffline()
{
    struct InFlightGuard
    {
        std::atomic<int>& flag;
        ~InFlightGuard() { flag.store(0, std::memory_order_release); }
    } inFlightGuard{ swingCacheRenderInFlight };

    juce::AudioBuffer<float> sourceCopy;
    juce::AudioBuffer<float> renderTarget;
    std::array<int, ModernAudioEngine::MaxColumns> sliceStarts{};
    double loopStartSamples = 0.0;
    double loopLength = 0.0;
    double beatsForLoop = 0.0;
    int loopColsClamped = 1;
    int loopStartCol = 0;
    float swing = 0.0f;
    int divisionClamped = 0;
    TimeStretchBackend stretchBackend{};
    bool useTransientAnchors = false;
    std::uint64_t anchorHash = 0;
    ...
    uint64_t sourceVersion = 0;
    {
        ...
        loopStartSamples = swingCacheRequestLoopStartSamples.load(std::memory_order_acquire);
        loopLength = swingCacheRequestLoopLength.load(std::memory_order_acquire);
        beatsForLoop = swingCacheRequestBeats.load(std::memory_order_acquire);
        loopColsClamped = juce::jlimit(1, ModernAudioEngine::MaxColumns,
                                       swingCacheRequestLoopCols.load(std::memory_order_acquire));
        loopStartCol = juce::jlimit(0, ModernAudioEngine::MaxColumns - 1,
                                    swingCacheRequestLoopStartCol.load(std::memory_order_acquire));
        requestSerial = swingCacheRequestSerial.load(std::memory_order_acquire);
        swing = juce::jlimit(0.0f, 1.0f, swingAmount.load(std::memory_order_acquire));
        stretchBackend = getStretchBackend();
        divisionClamped = juce::jlimit(0, static_cast<int>(SwingDivision::SixteenthTriplet),
                                       swingDivision.load(std::memory_order_acquire));
        useTransientAnchors = !transientSliceMode.load(std::memory_order_acquire);
        if (sampleBuffer.getNumSamples() <= 0 || sampleBuffer.getNumChannels() <= 0
            || !(beatsForLoop > 0.0) || swing <= 1.0e-4f)
        {
            swingCacheRenderRequested.store(0, std::memory_order_release);
            return false;
        }
        // KNOWN BOUNDED COST: rebuilding a dirty transient map here holds
        // bufferLock through a full-sample onset analysis (tens of ms on long
        // samples). It is rare — nearly all load paths install prepared
        // analysis so the map is not dirty — and matches the pre-existing
        // loader behavior. A fully unlocked analyze-on-copy path is the
        // follow-up if this ever shows up in practice.
        if (useTransientAnchors && transientSliceMapDirty)
            rebuildTransientSliceMap();
        sliceStarts = getSliceStartSamples(useTransientAnchors);
        anchorHash = 1469598103934665603ULL;
        anchorHash = hashMix64(anchorHash, static_cast<std::uint64_t>(useTransientAnchors ? 1 : 0));
        anchorHash = hashMix64(anchorHash, static_cast<std::uint64_t>(loopStartCol + 1));
        anchorHash = hashMix64(anchorHash, static_cast<std::uint64_t>(loopColsClamped + 1));
        for (int i = 0; i < ModernAudioEngine::MaxColumns; ++i)
    ...
    // Placement warp with the CAPTURED swing/division (the live members may
    // drift mid-render; the installed key must describe the values used).
    // Keep in sync with applySwingToPpq.
    const double warpUnitBeats = swingUnitBeatsFromDivision(static_cast<SwingDivision>(divisionClamped));
    const double warpPairLength = warpUnitBeats * 2.0;
    const double warpShapedSwing = std::pow(juce::jlimit(0.0, 1.0, static_cast<double>(swing)), 1.7);
    const double warpSplitShift = juce::jlimit(0.0, 0.72, warpShapedSwing * 0.72);
    const double warpSplitPoint = warpUnitBeats * (1.0 + warpSplitShift);
    auto warpBeat = [&](double ppq) -> double
    {
        if (warpPairLength <= 1.0e-9 || swing <= 1.0e-4f)
            return ppq;
        const double pairIndex = std::floor(ppq / warpPairLength);
        const double pairBase = pairIndex * warpPairLength;
        const double pairPhase = ppq - pairBase;
        double swungPhase = pairPhase;
        if (pairPhase < warpUnitBeats)
            swungPhase = warpSplitPoint * (pairPhase / juce::jmax(1.0e-9, warpUnitBeats));
        else
            swungPhase = warpSplitPoint
                + ((warpPairLength - warpSplitPoint) * ((pairPhase - warpUnitBeats) / juce::jmax(1.0e-9, warpUnitBeats)));
        return pairBase + swungPhase;
    };


    const int loopSamples = juce::jmax(1, static_cast<int>(std::llround(loopLength)));
    const int sampleCount = sourceCopy.getNumSamples();
    if (loopSamples <= 0 || sampleCount <= 0)
    {
        juce::ScopedLock relock(bufferLock);
        swingCacheRenderRequested.store(0, std::memory_order_release);
        return false;
    }
    ...
    renderTarget.setSize(2, loopSamples, false, true, true);
    renderTarget.clear();
    ...
    for (int i = 1; i < loopColsClamped; ++i)
    {
        ...
        const double swungBeat = warpBeat(unswungBeat);
    ...
    const int sampleRateInt = juce::jmax(8000, static_cast<int>(std::lround(capturedSampleRate)));
    ...
    auto writeSegmentSample = [&](int writeIndex, int segmentOffset, int blendSamples, float left, float right)
    {
        ...
        if (blendSamples > 0 && segmentOffset < blendSamples)
        {
            ...
            const float oldLeft = renderTarget.getSample(0, writeIndex);
            const float oldRight = renderTarget.getSample(1, writeIndex);
            left = oldLeft + ((left - oldLeft) * t);
            right = oldRight + ((right - oldRight) * t);
        }

        renderTarget.setSample(0, writeIndex, left);
        renderTarget.setSample(1, writeIndex, right);
    };
    for (int segment = 0; segment < segmentCount; ++segment)
    {
        ...
        for (int n = 0; n < inputLen; ++n)
        {
            ...
            const float left = linearReadWrapped(sourceCopy, 0, srcPos);
            const float right = (sourceCopy.getNumChannels() > 1)
                ? linearReadWrapped(sourceCopy, 1, srcPos)
                : left;
        ...
        const int maxBoundaryBlend = juce::jmax(
            4, static_cast<int>(std::lround(capturedSampleRate * 0.00075)));
        ...
        juce::AudioBuffer<float> sourceSegment(2, inputLen);
        for (int n = 0; n < inputLen; ++n)
        {
            sourceSegment.setSample(0, n, segmentIn[static_cast<size_t>(2 * n)]);
            sourceSegment.setSample(1, n, segmentIn[static_cast<size_t>((2 * n) + 1)]);
        }

        juce::AudioBuffer<float> stretchedSegment;
        if (!renderTimeStretchedBuffer(sourceSegment,
                                       static_cast<double>(sampleRateInt),
                                       targetLen,
                                       0.0f,
                                       stretchBackend,
                                       stretchedSegment))
        ...
        {
            const int receivedInt = stretchedSegment.getNumSamples();
            if (receivedInt <= 1)
            {
                const float left = (receivedInt > 0) ? stretchedSegment.getSample(0, 0) : 0.0f;
                const float right = (receivedInt > 0) ? stretchedSegment.getSample(1, 0) : 0.0f;
                for (int n = 0; n < targetLen; ++n)
                    writeSegmentSample(targetStart + n, n, boundaryBlendSamples, left, right);
                continue;
            }
            ...
            for (int n = 0; n < targetLen; ++n)
            {
                const double srcOutPos = (targetLen > 1 && receivedInt != targetLen)
                    ? (static_cast<double>(n) * static_cast<double>(receivedInt - 1)
                       / static_cast<double>(targetLen - 1))
                    : static_cast<double>(juce::jlimit(0, receivedInt - 1, n));
                ...
                const float frac = static_cast<float>(srcOutPos - static_cast<double>(s0));
                const float l0 = stretchedSegment.getSample(0, s0);
                const float l1 = stretchedSegment.getSample(0, s1);
                const float r0 = stretchedSegment.getSample(1, s0);
                const float r1 = stretchedSegment.getSample(1, s1);
                writeSegmentSample(targetStart + n, n, boundaryBlendSamples,
                                   l0 + ((l1 - l0) * frac),
                                   r0 + ((r1 - r0) * frac));
            }
    ...
    juce::ScopedLock lock(bufferLock);
    // Drop stale results (the request flag stays set so the poll reschedules).
    if (requestSerial != swingCacheRequestSerial.load(std::memory_order_acquire)
        || sourceVersion != pitchSourceVersion.load(std::memory_order_acquire))
    {
        return false;
    }

    soundTouchSwingCacheBuffer = std::move(renderTarget);
    soundTouchSwingCacheValid = soundTouchSwingCacheBuffer.getNumSamples() > 0;
    ...
    soundTouchSwingCacheDivision = divisionClamped;
    soundTouchSwingCacheLoopCols = loopColsClamped;
    soundTouchSwingCacheSourceSamples = sourceCopy.getNumSamples();
    soundTouchSwingCacheUsesTransientAnchors = useTransientAnchors;
    soundTouchSwingCacheAnchorHash = anchorHash;
    startPitchCacheOutputBlendLocked();
    swingCacheRenderRequested.store(0, std::memory_order_release);
    return soundTouchSwingCacheValid;
}
#endif
                                ...
                                double quantizeBeats)
{
    ...
    if (playMode == PlayMode::Sample && !isSampleModeLegacyLoopEngineEnabled())
    {
        playing = false;
        wasPlayingBeforeStop = false;
        renderPendingStoppedFadeBlockLocked(output, startSample, numSamples);
        return;
    }
    ...
    if (playMode == PlayMode::Step && hostJustStarted)
    {
        ...
        if (positionInfo.getPpqPosition().hasValue())
        {
            ...
            const float requestedTraversalRatio = (traversalSpeedOverrideActive.load(std::memory_order_acquire) != 0)
                ? traversalSpeedOverrideRatio.load(std::memory_order_acquire)
                : playheadSpeedRatio.load(std::memory_order_acquire);
            const double stepTraversalRatio = juce::jmax(
                0.125,
                static_cast<double>(requestedTraversalRatio));
                                                    ...
                                                    double swingMappedPositionInLoop = std::numeric_limits<double>::quiet_NaN()) -> double
    {
        ...
        const float requestedTraversalRatio = juce::jlimit(
            0.125f,
            8.0f,
            (traversalSpeedOverrideActive.load(std::memory_order_acquire) != 0)
                ? traversalSpeedOverrideRatio.load(std::memory_order_acquire)
                : playheadSpeedRatio.load(std::memory_order_acquire));
        ...
        const bool useContinuousTraversal = usesContinuousPlayheadTraversal();
        const double intraSlice = quantizedTraversalHandoffThisSample ? 0.0 : (slicePos - unswungBaseSlice);
        const double traversalSlicePhase = baseSliceForTraversal + intraSlice;
        const double speedPreservePhase = useContinuousTraversal ? traversalSlicePhase : baseSliceForTraversal;
        ...
        {
            // Preserve the currently audible traversal phase when speed changes.
            const double oldTraversalPos
                = (speedPreservePhase * playheadTraversalRatioAtLastCalc) + playheadTraversalPhaseOffsetSlices;
            playheadTraversalPhaseOffsetSlices = oldTraversalPos - (speedPreservePhase * playheadTraversalRatio);
            playheadTraversalRatioAtLastCalc = playheadTraversalRatio;
        }

        double traversedPosition = 0.0;
        if (useContinuousTraversal)
        {
            double traversedSlicePos = (traversalSlicePhase * playheadTraversalRatio) + playheadTraversalPhaseOffsetSlices;
            traversedSlicePos = std::fmod(traversedSlicePos, static_cast<double>(traversalSlices));
            if (traversedSlicePos < 0.0)
                traversedSlicePos += static_cast<double>(traversalSlices);
            traversedPosition = traversedSlicePos * sliceLength;
        }
        else
        {
            ...
            traversedPosition = (traversedSlice + intraSlice) * sliceLength;
        }
        if (traversalRearrangeActive.load(std::memory_order_acquire) != 0)
        {
            const float rearrangeDepth = juce::jlimit(0.0f, 1.0f,
                traversalRearrangeDepth.load(std::memory_order_acquire));
            if (rearrangeDepth > 1.0e-4f)
            {
                const int musicalSlices = juce::jmax(1, loopCols);
                const int targetLocalSlice = rearrangeSliceIndexFromNormalized(
                    traversalRearrangeValue.load(std::memory_order_acquire),
                    musicalSlices);
                auto armRearrangeBlend = [&]()
                {
                    const float rearrangeFadeMs = juce::jlimit(
                        0.3f,
                        8.0f,
                ...
                {
                    double wrappedTraversedPosition = std::fmod(traversedPosition, loopLenSafe);
                    if (wrappedTraversedPosition < 0.0)
                        wrappedTraversedPosition += loopLenSafe;

                    const int safeLoopStart = juce::jlimit(0, ModernAudioEngine::MaxColumns - 1, loopStart);
                    std::array<double, ModernAudioEngine::MaxColumns> localSliceStarts{};
                    std::array<double, ModernAudioEngine::MaxColumns> localSliceLengths{};
                    bool foundCurrentSlice = false;
                    int currentLocalSlice = 0;
                    double currentSliceOffsetSamples = 0.0;

                    for (int local = 0; local < musicalSlices; ++local)
                    {
                        const int currentGlobalColumn = juce::jlimit(
                            0,
                            ModernAudioEngine::MaxColumns - 1,
                            safeLoopStart + local);
                        const int nextLocalColumn = (local + 1) % musicalSlices;
                        const int nextGlobalColumn = juce::jlimit(
                            0,
                            ModernAudioEngine::MaxColumns - 1,
                            safeLoopStart + nextLocalColumn);

                        double currentSliceStart = std::fmod(
                            static_cast<double>(transientSliceSamples[static_cast<size_t>(currentGlobalColumn)])
                                - loopStartSamples,
                            loopLenSafe);
                        if (currentSliceStart < 0.0)
                            currentSliceStart += loopLenSafe;

                        double nextSliceStart = std::fmod(
                            static_cast<double>(transientSliceSamples[static_cast<size_t>(nextGlobalColumn)])
                                - loopStartSamples,
                            loopLenSafe);
                        if (nextSliceStart < 0.0)
                            nextSliceStart += loopLenSafe;
                        if (nextSliceStart <= currentSliceStart)
                            nextSliceStart += loopLenSafe;

                        const double currentSliceLength = juce::jmax(1.0, nextSliceStart - currentSliceStart);
                        localSliceStarts[static_cast<size_t>(local)] = currentSliceStart;
                        localSliceLengths[static_cast<size_t>(local)] = currentSliceLength;

                        double absoluteWrappedPosition = wrappedTraversedPosition;
                        if (absoluteWrappedPosition < currentSliceStart)
                            absoluteWrappedPosition += loopLenSafe;

                        if (!foundCurrentSlice
                            && absoluteWrappedPosition >= currentSliceStart
                            && absoluteWrappedPosition < nextSliceStart)
                        {
                            foundCurrentSlice = true;
                            currentLocalSlice = local;
                            currentSliceOffsetSamples = juce::jlimit(
                                0.0,
                                currentSliceLength - 1.0e-6,
                                absoluteWrappedPosition - currentSliceStart);
                        }
                    }
                    ...
                    if (foundCurrentSlice)
                    {
                        if (traversalRearrangeLastSourceSlice != currentLocalSlice
                            || traversalRearrangeLastTargetSlice != targetLocalSlice)
                        {
                            armRearrangeBlend();
                        }

                        traversalRearrangeLastSourceSlice = currentLocalSlice;
                        traversalRearrangeLastTargetSlice = targetLocalSlice;

                        const double targetSliceStart = localSliceStarts[static_cast<size_t>(targetLocalSlice)];
                        const double targetSliceLength = localSliceLengths[static_cast<size_t>(targetLocalSlice)];
                        const int targetCycle = (targetSliceLength > 1.0e-9)
                            ? juce::jmax(0, static_cast<int>(std::floor(currentSliceOffsetSamples / targetSliceLength)))
                            : 0;
                        if (traversalRearrangeLastTargetCycle != targetCycle)
                            armRearrangeBlend();
                        traversalRearrangeLastTargetCycle = targetCycle;

                        const double targetSliceOffset = (targetSliceLength > 1.0e-9)
                            ? std::fmod(currentSliceOffsetSamples, targetSliceLength)
                            : 0.0;
                        double targetSlicePosition = targetSliceStart + targetSliceOffset;
                        targetSlicePosition = std::fmod(targetSlicePosition, loopLenSafe);
                        if (targetSlicePosition < 0.0)
                            targetSlicePosition += loopLenSafe;

                        double delta = targetSlicePosition - wrappedTraversedPosition;
                        if (delta > (loopLenSafe * 0.5))
                            delta -= loopLenSafe;
                        else if (delta < -(loopLenSafe * 0.5))
                            delta += loopLenSafe;

                        traversedPosition = wrappedTraversedPosition + (delta * rearrangeDepth);
                        traversedPosition = std::fmod(traversedPosition, loopLenSafe);
                        if (traversedPosition < 0.0)
                            traversedPosition += loopLenSafe;
                    }
                    else
                    {
                ...
                {
                    const double rearrangeMusicalSliceLength = loopLenSafe / static_cast<double>(musicalSlices);
                    if (rearrangeMusicalSliceLength > 1.0e-9)
                    {
                        double localPos = std::fmod(traversedPosition / rearrangeMusicalSliceLength,
                                                    static_cast<double>(musicalSlices));
                        if (localPos < 0.0)
                            localPos += static_cast<double>(musicalSlices);

                        const int currentLocalSlice = juce::jlimit(
                            0,
                            musicalSlices - 1,
                            static_cast<int>(std::floor(localPos)));
                        if (traversalRearrangeLastSourceSlice != currentLocalSlice
                            || traversalRearrangeLastTargetSlice != targetLocalSlice)
                        {
                            armRearrangeBlend();
                        ...
                        const double intraLocal = localPos - std::floor(localPos);
                        double targetLocalPos = static_cast<double>(targetLocalSlice) + intraLocal;
                        double blendedLocalPos = localPos + ((targetLocalPos - localPos) * rearrangeDepth);
                        blendedLocalPos = std::fmod(blendedLocalPos, static_cast<double>(musicalSlices));
                        if (blendedLocalPos < 0.0)
                            blendedLocalPos += static_cast<double>(musicalSlices);
                        traversedPosition = blendedLocalPos * rearrangeMusicalSliceLength;
                    }
                }
            }
            else
    ...
    bool loopTempoMatchActive = false;
    const juce::AudioBuffer<float>* playbackSourceBuffer = &sampleBuffer;
    double playbackSourcePositionScale = 1.0;
    double playbackSourceLoopStartSamples = loopStartSamples;
    double playbackSourceSampleLength = sampleLength;
    double playbackSourceLoopLength = juce::jmax(1.0, loopLength);
#if MLRVST_ENABLE_BUNGEE
    const bool canUseLoopTempoMatchCache = !isScratching
        && (playMode == PlayMode::Loop
            || playMode == PlayMode::Gate
            || playMode == PlayMode::OneShot
            || playMode == PlayMode::Grain)
        && rebuildLoopTempoMatchCache(tempo, beatsForLoop);
    if (canUseLoopTempoMatchCache)
    {
        loopTempoMatchActive = true;
        playbackSourceBuffer = &loopTempoMatchCacheBuffer;
        playbackSourcePositionScale = juce::jmax(1.0e-9, loopTempoMatchCachePositionScale);
        playbackSourceLoopStartSamples = loopStartSamples * playbackSourcePositionScale;
        playbackSourceSampleLength = static_cast<double>(loopTempoMatchCacheBuffer.getNumSamples());
        playbackSourceLoopLength = juce::jmax(1.0, loopLength * playbackSourcePositionScale);
    }
#endif
#if MLRVST_ENABLE_SOUNDTOUCH || MLRVST_ENABLE_BUNGEE
    if (!loopTempoMatchActive
        && shouldUseSoundTouchSwingCache(loopLength, beatsForLoop, loopCols, isScratching, playheadTraversalRatio)
    ...
    if (playMode == PlayMode::Step)
    {
        ...
        else if (std::abs(stepTraversalRatio - stepTraversalRatioAtLastTick) > 1.0e-6)
        {
            ...
            if (positionInfo.getPpqPosition().hasValue() && tempo > 0.0)
            {
                const double ppqNow = applySwingInverseToPpq(*positionInfo.getPpqPosition());
        ...
        if (stepStutterTimingActive)
        {
            ...
            if (positionInfo.getPpqPosition().hasValue() && tempo > 0.0 && currentSampleRate > 0.0)
            {
                ...
                const double ppqAtSegmentEnd = applySwingInverseToPpq(ppqAtSegmentEndRaw);
        ...
        else if (positionInfo.getPpqPosition().hasValue() && tempo > 0.0)
        {
            ...
            while (processed < numSamples)
            {
                ...
                const double ppqAtProcessed = applySwingInverseToPpq(ppqAtProcessedRaw);
                ...
                // nextBoundaryPpq lives in the inverse-warp domain; map it
                // through the forward warp to raw host ppq before converting
                // the distance to samples, so boundaries land exactly.
                const double samplesToBoundary =
                    (applySwingToPpq(nextBoundaryPpq) - ppqAtProcessedRaw) * samplesPerBeatLocal;
                    ...
                    && stepTraversalTick == stepSubdivisionSixteenth)
                {
                    ...
                    const double ppqAfterProcessed = applySwingInverseToPpq(ppqAfterProcessedRaw);
        ...
        // Old-mode output parked in the stopped fader (mode switch into Step)
        // still fades out under the step output instead of being cut.
        renderPendingStoppedFadeBlockLocked(output, startSample, numSamples);
    ...
    for (int i = 0; i < numSamples; ++i)
    {
        ...
        float uiDisplaySpeed = juce::jlimit(0.0f, 8.0f, std::abs(static_cast<float>(rateMultiplier)));
        if (playMode == PlayMode::Grain)
        {
            const float grainScratch = scratchAmount.load(std::memory_order_acquire);
            if (grainScratch <= 0.001f)
            ...
            else
            {
                ...
                const float baseDisplay = juce::jlimit(0.1f, 8.0f, std::abs(static_cast<float>(playbackSpeed.load(std::memory_order_acquire))));
                const float expFalloff = static_cast<float>(std::exp(-4.2 * progress));
                uiDisplaySpeed = (remain < 1.0) ? 0.0f : juce::jlimit(0.0f, 8.0f, baseDisplay * expFalloff);
            }
        }
        else if (scrubActive && stripScratch > 0.0f)
        {
            uiDisplaySpeed = juce::jlimit(0.0f, 8.0f, std::abs(static_cast<float>(scratchRate)));
        }
        ...
        // Full-loop seam declick: with no pre-roll content before the loop
        // start, a content crossfade is impossible (it degenerates into a
        // correlated self-blend). Instead, when the mapped position jumps
        // across the seam, blend the wrap from the recent output tail — the
        // same mechanism retrigger blends use.
        const bool seamHasPrerollContent =
            playbackSourceLoopStartSamples >= crossfadeSamples * playbackSourcePositionScale;
        if (playMode == PlayMode::Loop
            && !loopSliceEnvelopeActive
            && !soundTouchSwingActive
            && !scratchBypassPpq
            && crossfadeSamples > 0
            && crossfadeSamples < loopLength
            && !seamHasPrerollContent)
        {
            if (lastSeamPositionInLoop >= 0.0
                && std::abs(positionInLoop - lastSeamPositionInLoop) > loopLength * 0.5)
            {
                armTriggerOutputBlendLocked(
                    juce::jmax(16, static_cast<int>(crossfadeSamples)),
                    true);
            }
            lastSeamPositionInLoop = positionInLoop;
        }
        else
        {
            lastSeamPositionInLoop = -1.0;
        }

        // Crossfade the loop seam only when a REAL pre-roll region exists
        // before the loop start (inner loops); full loops use the tail blend
        // above.
        if (!loopSliceEnvelopeActive
            && !soundTouchSwingActive
            && crossfadeSamples > 0
            ...
            && seamHasPrerollContent)
        {
            ...
            if (positionInLoop >= fadeStart)
            {
                ...
                prerollSamplePosition = playbackSourceLoopStartSamples
                    + ((positionInLoop - loopLength) * playbackSourcePositionScale);
            ...
            && playMode != PlayMode::Grain)
        {
            ...
            const double oldPositionInLoop = oldPos - loopStartSamples;
            const double oldReadPos = mapResampleReadPosition(
                playbackSourceLoopStartSamples + (oldPositionInLoop * playbackSourcePositionScale),
                oldPositionInLoop);
                                         ...
                                         double stutterOffsetRatioOverride)
{
    ...
    if (wasPlaying && sampleLength > 1.0 && playMode != PlayMode::Step && playMode != PlayMode::Grain)
    {
        ...
        {
            triggerFadeMs = juce::jmax(triggerFadeMs, transientModeActive ? 18.0f : 24.0f);
        }
        if (retriggerBlendActive && retriggerBlendSamplesRemaining > 0 && retriggerBlendTotalSamples > 0)
        {
            const float progress = 1.0f - (static_cast<float>(retriggerBlendSamplesRemaining)
    ...
    const bool retriggerDuringStopFade = stopAfterFade;
    stopAfterFade = false;
    playing = true;
    ...
    if (!wasPlaying)
        crossfader.startFade(true, fadeSamples, true);
    else if (retriggerDuringStopFade)
    {
        // Retrigger landed while a stop/choke fade-out was running: resume the
        // fade-in from the current gain instead of letting the orphaned
        // fade-out finish and snap 0 -> 1 in a single sample.
        crossfader.startFade(true, fadeSamples, false);
    }
}
...
void EnhancedAudioStrip::captureMomentaryPhaseReference(double hostPpq)
{
    ...
    momentaryPhaseSavedPpqTimelineAnchored = ppqTimelineAnchored;
    momentaryPhaseSavedPpqTimelineOffsetBeats = ppqTimelineOffsetBeats;
    momentaryPhaseSavedTriggerSample = triggerSample;
    momentaryPhaseSavedTriggerPpqPosition = triggerPpqPosition;
    momentaryPhaseSavedLastTriggerPpq = lastTriggerPPQ;
    momentaryPhaseSavedTriggerOffsetRatio = triggerOffsetRatio;
    momentaryPhaseSavedTriggerColumn = triggerColumn;

    double offset = 0.0;
    if (ppqTimelineAnchored && std::isfinite(hostPpq))
    {
        offset = std::fmod(ppqTimelineOffsetBeats, beatsForLoop);
        if (offset < 0.0)
            offset += beatsForLoop;
    }
    else
    {
        ...
        offset = std::fmod(beatInLoop - hostPpq, beatsForLoop);
        if (offset < 0.0)
            offset += beatsForLoop;
    }
...
void EnhancedAudioStrip::enforceMomentaryPhaseReference(double hostPpq, int64_t currentGlobalSample)
{
    ...
    ppqTimelineAnchored = momentaryPhaseSavedPpqTimelineAnchored;
    if (ppqTimelineAnchored)
    {
        ppqTimelineOffsetBeats = std::fmod(momentaryPhaseSavedPpqTimelineOffsetBeats, beatsForLoop);
        if (ppqTimelineOffsetBeats < 0.0)
            ppqTimelineOffsetBeats += beatsForLoop;
    }
    else
    ...
    triggerColumn = juce::jlimit(0, ModernAudioEngine::MaxColumns - 1, momentaryPhaseSavedTriggerColumn);
    triggerSample = momentaryPhaseSavedTriggerSample;
    triggerPpqPosition = momentaryPhaseSavedTriggerPpqPosition;
    lastTriggerPPQ = momentaryPhaseSavedLastTriggerPpq;
    triggerOffsetRatio = juce::jlimit(0.0, 0.999999, momentaryPhaseSavedTriggerOffsetRatio);

    const double expectedPos = juce::jlimit(0.0,
                                            juce::jmax(0.0, sampleLength - 1.0),
                                            getTimelinePositionForSample(currentGlobalSample));
    playbackPosition = expectedPos;
    stopLoopPosition = expectedPos;
    ...
    if (ppqTimelineAnchored && std::isfinite(hostPpq))
    {
        ...
        triggerOffsetRatio = juce::jlimit(0.0, 0.999999, beatInLoop / beatsForLoop);
    }
...
void EnhancedAudioStrip::stop(bool immediate)
{
    ...
    else
    {
        ...
        int fadeSamples = juce::jmax(256, static_cast<int>(currentSampleRate * 0.012)); // ~12ms
        stopAfterFade = true;
...
void EnhancedAudioStrip::retriggerStepVoiceAtColumn(int column, bool forceColumn)
{
    ...
    const int totalSteps = juce::jmax(1, getStepTotalSteps());
    const int currentStepValue = currentStep;
    const int previousStep = juce::jlimit(0, totalSteps - 1, currentStepValue);
    const int pageCount = juce::jmax(1, (totalSteps + (ModernAudioEngine::MaxColumns - 1)) / ModernAudioEngine::MaxColumns);
    const int currentPage = juce::jlimit(0, pageCount - 1, currentStepValue / ModernAudioEngine::MaxColumns);
    ...
    const bool authoredStepEnabled = stepPattern[static_cast<size_t>(triggerStepIndex)];
    if (!forceColumn && !authoredStepEnabled)
    {
    ...
    if (totalSteps > ModernAudioEngine::MaxColumns)
        setStepPage(triggerStepIndex / ModernAudioEngine::MaxColumns);

    stepSubdivisionTriggerIndex = 0;
    stepSubdivisionSixteenth = std::numeric_limits<int64_t>::min();
    stepSubdivisionGateOpen = stepPattern[static_cast<size_t>(triggerStepIndex)];
    if (!stepSubdivisionGateOpen)
        return;
...
{
    // Loop geometry is read per-sample by process(); tear-free updates only.
    juce::ScopedLock lock(bufferLock);
                                               ...
                                               const ContinuityBlendState* continuityState)
{
    ...
    if (!timelineAnchored || !std::isfinite(timelineOffsetBeats) || tempo <= 0.0 || !std::isfinite(currentTimelineBeat))
    {
        ...
        if (std::isfinite(currentTimelineBeat))
...
void EnhancedAudioStrip::setTempoMatchBackend(TimeStretchBackend backend)
{
    const int newValue = static_cast<int>(sanitizeTimeStretchBackend(static_cast<int>(backend)));
    const int oldValue = tempoMatchBackend.exchange(newValue, std::memory_order_acq_rel);
    if (oldValue == newValue)
        return;
#if MLRVST_ENABLE_BUNGEE
    invalidateLoopTempoMatchCache();
...
{
    setStretchBackend(enabled ? TimeStretchBackend::SoundTouch
...
void EnhancedAudioStrip::renderPendingStoppedFadeBlockLocked(juce::AudioBuffer<float>& output,
                                                             int startSample,
                                                             int numSamples)
{
    // Mode switches and early-return render paths park their old output in
    // stoppedOutputSampleFader; without this flush the tail is silently cut.
    if (!stoppedOutputSampleFader.isActive())
        return;
    ...
    for (int i = 0; i < numSamples; ++i)
    {
        float fadeL = 0.0f;
        float fadeR = 0.0f;
        if (!renderStoppedOutputFadeSampleLocked(fadeL, fadeR))
            break;

        output.addSample(0, startSample + i, fadeL);
        if (numChannels > 1)
            output.addSample(1, startSample + i, fadeR);
    }
}
...
void EnhancedAudioStrip::startPitchCacheOutputBlendLocked()
{
    ...
    const float triggerFadeMs = juce::jlimit(0.01f, 120.0f, triggerFadeInMs.load(std::memory_order_acquire));
    const float blendMs = juce::jlimit(1.0f, 12.0f, juce::jmax(1.0f, triggerFadeMs));
    pitchCacheOutputBlendTotalSamples = juce::jmax(16, static_cast<int>(std::round(currentSampleRate * 0.001 * blendMs)));
...
double EnhancedAudioStrip::applySwingInverseToPpq(double ppq) const
{
    const double swing = static_cast<double>(swingAmount.load(std::memory_order_acquire));
    if (swing <= 1.0e-4)
        return ppq;
    ...
    // Both segments of the forward warp are linear with positive slope, so
    // the inverse is exact and piecewise-linear as well.
    double rawPhase = pairPhase;
    if (pairPhase < splitPoint)
    {
        rawPhase = (pairPhase / juce::jmax(1.0e-9, splitPoint)) * unitBeats;
    }
    else
    {
        const double t = (pairPhase - splitPoint) / juce::jmax(1.0e-9, pairLength - splitPoint);
        rawPhase = unitBeats + (t * unitBeats);
    }

    return pairBase + rawPhase;
}
...
void EnhancedAudioStrip::setTraversalSpeedOverride(float ratio)
{
    traversalSpeedOverrideRatio.store(juce::jlimit(0.125f, 8.0f, ratio), std::memory_order_release);
    traversalSpeedOverrideActive.store(1, std::memory_order_release);
}

void EnhancedAudioStrip::clearTraversalSpeedOverride()
{
    traversalSpeedOverrideActive.store(0, std::memory_order_release);
    traversalSpeedOverrideRatio.store(1.0f, std::memory_order_release);
}

double EnhancedAudioStrip::getLoopPhaseNormalized() const
{
    if (sampleLength <= 0.0)
        return 0.0;
...
ModernAudioEngine::ModernAudioEngine()
{
    ...
    for (int stripIndex = 0; stripIndex < MaxStrips; ++stripIndex)
    {
        ...
        for (int slot = 0; slot < NumModSequencers; ++slot)
            resetModSequencerSlotToDefaults(stripIndex, slot);
    }
...
{
    globalSampleCount.store(juce::jmax<int64_t>(0, globalSamplePosition), std::memory_order_release);
    quantizeClock.seedTimelineState(globalSamplePosition, ppqPosition);
    if (std::isfinite(ppqPosition))
    {
        hasLastKnownPPQ.store(true, std::memory_order_release);
        lastKnownPPQ.store(ppqPosition, std::memory_order_release);
    }
    else
    {
        hasLastKnownPPQ.store(false, std::memory_order_release);
        lastKnownPPQ.store(0.0, std::memory_order_release);
    }
}
                                     ...
                                     const std::array<juce::AudioBuffer<float>*, MaxStrips>* stripOutputs)
{
    ...
    // Some hosts deliver blocks larger than the prepareToPlay estimate
    // (offline bounces in particular). Grow the duck scratch storage before
    // any duck work touches it — one rare allocation beats a heap overflow.
    if (!duckStripScratchBuffers.empty()
        && numSamples > duckStripScratchBuffers[0].getNumSamples())
    {
        jassertfalse; // host sent a block larger than it promised in prepareToPlay
        for (size_t i = 0; i < duckStripScratchBuffers.size(); ++i)
        {
            duckStripScratchBuffers[i].setSize(2, numSamples, false, false, true);
            duckDetectorEnvelopeBuffers[i].setSize(1, numSamples, false, false, true);
        }
    }
    ...
    auto processStripsSegment = [&](int startSample, int segmentSamples)
    {
        ...
        auto publishLaneRuntimeSnapshot = [](ModSequencer& laneSeq, const ModSequencerRuntime& laneRuntime)
        {
            laneSeq.runtimeRawSnapshot.store(laneRuntime.smoothedRaw, std::memory_order_release);
            laneSeq.runtimeGrainSnapshot.store(laneRuntime.grainDezipperedRaw, std::memory_order_release);
            laneSeq.runtimePitchSnapshot.store(laneRuntime.pitchDezipperedRaw, std::memory_order_release);
            laneSeq.runtimeSpeedTargetSnapshot.store(laneRuntime.speedDezipperedTarget, std::memory_order_release);
        };

        auto applyPendingLaneRuntimeReset = [&](ModSequencer& laneSeq, ModSequencerRuntime& laneRuntime)
        {
            const auto requestedGeneration = laneSeq.runtimeResetGeneration.load(std::memory_order_acquire);
            if (laneRuntime.appliedResetGeneration == requestedGeneration)
                return;

            laneRuntime.smoothedRaw = laneSeq.runtimeResetSeedRaw.load(std::memory_order_acquire);
            laneRuntime.grainDezipperedRaw = laneSeq.runtimeResetSeedGrain.load(std::memory_order_acquire);
            laneRuntime.pitchDezipperedRaw = laneSeq.runtimeResetSeedPitch.load(std::memory_order_acquire);
            laneRuntime.speedDezipperedTarget = laneSeq.runtimeResetSeedSpeedTarget.load(std::memory_order_acquire);
            laneRuntime.syncPhaseLast = -1.0;
            laneRuntime.syncCycleCount = 0;
            laneRuntime.syncPhaseValid = false;
            laneRuntime.appliedResetGeneration = requestedGeneration;
            publishLaneRuntimeSnapshot(laneSeq, laneRuntime);
        };

        auto resetLaneRuntime = [&](ModSequencer& laneSeq, ModSequencerRuntime& laneRuntime)
        {
            laneRuntime.smoothedRaw = 0.0f;
            laneRuntime.grainDezipperedRaw = 0.0f;
            laneRuntime.pitchDezipperedRaw = 0.0f;
            laneRuntime.speedDezipperedTarget = -1.0f;
            publishLaneRuntimeSnapshot(laneSeq, laneRuntime);
        };

        auto resetLaneClock = [](ModSequencerRuntime& laneRuntime)
        {
            laneRuntime.syncPhaseValid = false;
            laneRuntime.syncPhaseLast = -1.0;
            laneRuntime.syncCycleCount = 0;
        ...
        for (size_t i = 0; i < static_cast<size_t>(MaxStrips); ++i)
        {
            ...
            // (Rearrange mod lanes were removed; the target survives only for
            // serialization compatibility and is sanitized to None on load.)
            ...
            float originalLoopSliceLength = 1.0f;
            float originalScratchAmount = 0.0f;
            ...
            float originalGrainPositionJitter = 0.0f;
            ...
            float originalGrainShape = 0.0f;
            bool originalFilterEnabled = false;
            ...
            if (stripPlaying)
            {
                ...
                originalFilterEnabled = strip->isFilterEnabled();
                ...
                originalLoopSliceLength = strip->getLoopSliceLength();
                originalScratchAmount = strip->getScratchAmount();
                ...
                originalGrainPositionJitter = strip->getGrainPositionJitter();
                ...
                originalGrainShape = strip->getGrainShape();
                ...
                if (stepSampler != nullptr)
                {
                    ...
                    originalStepFilterType = stepSampler->getFilterType();
                }
            }
            ...
            auto resolveLanePhase = [&](ModSequencer& laneSeq,
                                        ModSequencerRuntime& laneRuntime,
                                        ...
                                        double& stepPhaseOut) -> bool
            {
                ...
                if (laneTransportMode == ModTransportMode::Sync)
                {
                    ...
                    if (std::isfinite(loopPhase))
                    {
                        ...
                        if (!laneRuntime.syncPhaseValid)
                        {
                            laneRuntime.syncPhaseValid = true;
                            laneRuntime.syncPhaseLast = wrappedPhase;
                            laneRuntime.syncCycleCount = 0;
                        }
                        else
                        {
                            if ((laneRuntime.syncPhaseLast - wrappedPhase) > 0.5)
                                ++laneRuntime.syncCycleCount;
                            else if ((wrappedPhase - laneRuntime.syncPhaseLast) > 0.85)
                                laneRuntime.syncCycleCount = 0;
                            laneRuntime.syncPhaseLast = wrappedPhase;
                        }

                        const double stepsPos = (((static_cast<double>(laneRuntime.syncCycleCount) + wrappedPhase)
                ...
                else if (laneTransportMode == ModTransportMode::Scene && modulationPpqReady)
                {
                    const bool sceneClockReady = sceneModulationContextEnabled.load(std::memory_order_acquire) != 0;
                    const double sceneStartPpq = sceneModulationStartPpq.load(std::memory_order_acquire);
                    const double sceneLengthBeats = sceneModulationLengthBeats.load(std::memory_order_acquire);
                    if (sceneClockReady
                        && std::isfinite(sceneStartPpq)
                        && std::isfinite(sceneLengthBeats)
                        && sceneLengthBeats > 1.0e-6)
                    {
                        laneRuntime.syncPhaseValid = false;
                        laneRuntime.syncPhaseLast = -1.0;
                        laneRuntime.syncCycleCount = 0;
                        const double stepPpq = basePpq + (static_cast<double>(startSample) / juce::jmax(1.0, samplesPerBeat));
                        double sceneBeat = std::fmod(stepPpq - sceneStartPpq, sceneLengthBeats);
                        if (sceneBeat < 0.0)
                            sceneBeat += sceneLengthBeats;
                        const double scenePhase = juce::jlimit(0.0, 1.0, sceneBeat / sceneLengthBeats);
                        const double stepsPos = ((scenePhase * static_cast<double>(totalSteps)) * static_cast<double>(laneRate))
                            + static_cast<double>(offset);
                        const double wrapped = std::fmod(stepsPos, static_cast<double>(totalSteps));
                        const double wrappedPos = (wrapped < 0.0) ? (wrapped + static_cast<double>(totalSteps)) : wrapped;
                        globalStepOut = juce::jlimit(0, totalSteps - 1, static_cast<int>(std::floor(wrappedPos)));
                ...
                if (!updatedFromClock
                    && modulationPpqReady
                    && laneTransportMode != ModTransportMode::Scene)
                {
                    laneRuntime.syncPhaseValid = false;
                    laneRuntime.syncPhaseLast = -1.0;
                    laneRuntime.syncCycleCount = 0;
            ...
            for (int slot = 0; slot < NumModSequencers; ++slot)
            {
                ...
                auto& runtime = modSequencerRuntime[i][static_cast<size_t>(slot)];
                applyPendingLaneRuntimeReset(seq, runtime);
                ...
                if (!stripPlaying)
                {
                    resetLaneClock(runtime);
                    resetLaneRuntime(seq, runtime);
                    continue;
                }
                ...
                {
                    // Audible retriggers are driven by the scheduler, but the
                    // lane clock must still advance so the GUI playhead, arc
                    // step selection, and monome page sync stay live.
                    const int retrigLengthBars = juce::jlimit(1, MaxModBars, seq.lengthBars.load(std::memory_order_acquire));
                    const int retrigTotalSteps = juce::jmax(ModSteps, ModSteps * retrigLengthBars);
                    const int retrigOffset = seq.offset.load(std::memory_order_acquire);
                    int retrigGlobalStep = seq.lastGlobalStep.load(std::memory_order_acquire);
                    double retrigStepPhase = 0.0;
                    resolveLanePhase(seq, runtime, transportMode, modRate,
                                     retrigOffset, retrigTotalSteps, retrigGlobalStep, retrigStepPhase);
                    continue;
                }

                if (target == ModTarget::Rearrange)
                {
                    // Rearrange mod lanes were removed from the product; a
                    // legacy-serialized lane just idles.
                    resetLaneRuntime(seq, runtime);
                    continue;
                }
                ...
                if (!resolveLanePhase(seq, runtime, transportMode, modRate, offset, totalSteps, globalStep, stepPhase))
                {
                    resetLaneRuntime(seq, runtime);
                    continue;
                }
                ...
                if (smoothingMs > 0.0f)
                {
                    ...
                    runtime.smoothedRaw += (shapedRaw - runtime.smoothedRaw) * juce::jlimit(0.0f, 1.0f, alpha);
                    smoothedRaw = runtime.smoothedRaw;
                }
                else
                {
                    runtime.smoothedRaw = shapedRaw;
                }
                ...
                if (isGrainModTarget(target))
                {
                    ...
                    runtime.grainDezipperedRaw += (smoothedRaw - runtime.grainDezipperedRaw) * juce::jlimit(0.0f, 1.0f, alpha);
                    controlRaw = runtime.grainDezipperedRaw;
                }
                else
                {
                    runtime.grainDezipperedRaw = smoothedRaw;
                }
                ...
                if (target == ModTarget::Pitch)
                {
                    ...
                    const float alpha = 1.0f - std::exp(-static_cast<float>(segmentSamples) / dezipSamples);
                    runtime.pitchDezipperedRaw += (controlRaw - runtime.pitchDezipperedRaw) * juce::jlimit(0.0f, 1.0f, alpha);
                    controlRaw = runtime.pitchDezipperedRaw;
                }
                else
                {
                    runtime.pitchDezipperedRaw = controlRaw;
                }
                ...
                if (target != ModTarget::Speed)
                    runtime.speedDezipperedTarget = -1.0f;

                const float depth = juce::jlimit(0.0f, 1.0f, seq.depth.load(std::memory_order_acquire));
                const bool bipolar = modTargetSupportsBipolar(target)
                ...
                switch (target)
                {
                    ...
                    case ModTarget::Pan:
                    {
                        // Bipolar offset around the user's pan: step 0.5 is a
                        // true no-op at any depth (the old absolute blend
                        // dragged every strip toward center by default).
                        const float panOffset = ((controlRaw * 2.0f) - 1.0f) * depth;
                        const float targetPan = juce::jlimit(-1.0f, 1.0f, originalPan + panOffset);
                    ...
                    {
                        const float speedControl = bipolar
                            ? juce::jlimit(0.0f, 1.0f, 0.5f + (0.5f * modBi))
                            : juce::jlimit(0.0f, 1.0f, modNorm);
                        const float speedRatio = quantizeSpeedRatioMusical(speedControl, bipolar);
                        const float speedBase = strip->getPlayheadSpeedRatio();
                        const float targetSpeed = juce::jlimit(0.125f, 8.0f, speedBase * speedRatio);
                        strip->setTraversalSpeedOverride(targetSpeed);
                        break;
                    }
                    ...
                    {
                        const float targetMorph = juce::jlimit(0.0f, 1.0f, originalFilterMorph + modSigned);
                        strip->setFilterMorph(targetMorph);
                        if (stepSampler != nullptr)
                        {
                            stepSampler->setFilterEnabled(true);
                            FilterType stepType = FilterType::LowPass;
                            if (targetMorph >= 0.75f)
                                stepType = FilterType::HighPass;
                            else if (targetMorph >= 0.25f)
                    ...
                    case ModTarget::GrainPositionJitter:
                        strip->setGrainPositionJitter(juce::jlimit(0.0f, 1.0f, originalGrainPositionJitter + (0.5f * modSigned)));
                        break;
                    ...
                    {
                        const float baseNorm = juce::jlimit(0.0f,
                                                            1.0f,
                                                            (originalLoopSliceLength - 0.02f) / 0.98f);
                        const float targetNorm = juce::jlimit(0.0f, 1.0f, baseNorm + modSigned);
                        strip->setLoopSliceLength(0.02f + (0.98f * targetNorm));
                        break;
                    }
                    case ModTarget::Scratch:
                    {
                        const float baseNorm = juce::jlimit(0.0f, 1.0f, originalScratchAmount / 100.0f);
                        const float targetNorm = juce::jlimit(0.0f, 1.0f, baseNorm + modSigned);
                        strip->setScratchAmount(targetNorm * 100.0f);
                        break;
                    }
                    case ModTarget::None:
                    default:
                ...
                publishLaneRuntimeSnapshot(seq, runtime);
            }

            // Post-apply snapshot for the guarded restore below: if a param no
            // longer holds the value the mod system wrote (the message thread
            // changed it during this segment), the user's write wins and the
            // next segment adopts it as the new base. This replaces the old
            // blanket restore, which silently reverted concurrent knob edits
            // (and rewrote params no lane even touched).
            float appliedVol = 0.0f, appliedPan = 0.0f, appliedSpeed = 0.0f, appliedPitch = 0.0f;
            float appliedFilterFreq = 0.0f, appliedFilterRes = 0.0f, appliedFilterMorph = 0.0f;
            float appliedDelayMix = 0.0f, appliedDelayTimeBeats = 0.0f, appliedDelayFeedback = 0.0f;
            float appliedDelayLowCutHz = 0.0f, appliedDelayHighCutHz = 0.0f;
            float appliedLoopSliceLength = 0.0f, appliedScratchAmount = 0.0f;
            float appliedGrainDensity = 0.0f, appliedGrainPitch = 0.0f, appliedGrainPitchJitter = 0.0f;
            float appliedGrainSpread = 0.0f, appliedGrainJitter = 0.0f, appliedGrainPositionJitter = 0.0f;
            float appliedGrainRandom = 0.0f, appliedGrainArp = 0.0f, appliedGrainCloud = 0.0f;
            float appliedGrainEmitter = 0.0f, appliedGrainEnvelope = 0.0f, appliedGrainShape = 0.0f;
            bool appliedFilterEnabled = false;
            float appliedStepVol = 0.0f, appliedStepPan = 0.0f, appliedStepSpeed = 0.0f;
            float appliedStepFilterFreq = 0.0f, appliedStepFilterRes = 0.0f;
            bool appliedStepFilterEnabled = false;
            FilterType appliedStepFilterType = FilterType::LowPass;
            if (anyParamModApplied)
            {
                appliedVol = strip->getVolume();
                appliedPan = strip->getPan();
                appliedSpeed = strip->getPlaybackSpeed();
                appliedPitch = strip->getPitchShift();
                appliedFilterEnabled = strip->isFilterEnabled();
                appliedFilterFreq = strip->getFilterFrequency();
                appliedFilterRes = strip->getFilterResonance();
                appliedFilterMorph = strip->getFilterMorph();
                appliedDelayMix = strip->getDelayMix();
                appliedDelayTimeBeats = strip->getDelayTimeBeats();
                appliedDelayFeedback = strip->getDelayFeedback();
                appliedDelayLowCutHz = strip->getDelayLowCutHz();
                appliedDelayHighCutHz = strip->getDelayHighCutHz();
                appliedLoopSliceLength = strip->getLoopSliceLength();
                appliedScratchAmount = strip->getScratchAmount();
                appliedGrainDensity = strip->getGrainDensity();
                appliedGrainPitch = strip->getGrainPitch();
                appliedGrainPitchJitter = strip->getGrainPitchJitter();
                appliedGrainSpread = strip->getGrainSpread();
                appliedGrainJitter = strip->getGrainJitter();
                appliedGrainPositionJitter = strip->getGrainPositionJitter();
                appliedGrainRandom = strip->getGrainRandomDepth();
                appliedGrainArp = strip->getGrainArpDepth();
                appliedGrainCloud = strip->getGrainCloudDepth();
                appliedGrainEmitter = strip->getGrainEmitterDepth();
                appliedGrainEnvelope = strip->getGrainEnvelope();
                appliedGrainShape = strip->getGrainShape();
                if (stepSampler != nullptr)
                {
                    appliedStepVol = stepSampler->getVolume();
                    appliedStepPan = stepSampler->getPan();
                    appliedStepSpeed = std::pow(2.0f, static_cast<float>(stepSampler->getPitchOffset()) / 12.0f);
                    appliedStepFilterFreq = stepSampler->getFilterFrequency();
                    appliedStepFilterRes = stepSampler->getFilterResonance();
                    appliedStepFilterEnabled = stepSampler->isFilterEnabled();
                    appliedStepFilterType = stepSampler->getFilterType();
                }
            }

            juce::AudioBuffer<float>* targetBuffer = useDuckProcessing
                ? &duckStripScratchBuffers[i]
                : resolveFinalTargetBuffer(buffer, stripOutputs, i);
            ...
            auto* stripStepSampler = strip->getPlayMode() == EnhancedAudioStrip::PlayMode::Step
                ? strip->getStepSampler()
                : nullptr;
            ...
            if (groupId >= 0 && groupId < MaxGroups && groups[static_cast<size_t>(groupId)])
            {
                ...
                if (!group->isMuted())
                {
                    ...
                    // The Step render path bypasses the strip volume entirely;
                    // apply the group gain inside the step sampler.
                    if (stripStepSampler != nullptr)
                        stripStepSampler->setGroupGain(groupVol);
                    renderCurrentStrip();
            ...
            {
                if (stripStepSampler != nullptr)
                    stripStepSampler->setGroupGain(1.0f);
                renderCurrentStrip();
            }
            ...
            if (anyParamModApplied)
            {
                // Guarded restore: put the base value back only when the param
                // still holds what the mod system wrote. If it changed during
                // the segment, the message thread wrote it — keep the user's
                // value and let the next segment's capture adopt it as base.
                auto restoreIf = [](float live, float applied, auto&& restoreFn)
                {
                    if (live == applied)
                        restoreFn();
                };
                restoreIf(strip->getVolume(), appliedVol, [&]{ strip->setVolume(originalVol); });
                restoreIf(strip->getPan(), appliedPan, [&]{ strip->setPan(originalPan); });
                restoreIf(strip->getPlaybackSpeed(), appliedSpeed, [&]{ strip->setPlaybackSpeedImmediate(originalSpeed); });
                restoreIf(strip->getPitchShift(), appliedPitch, [&]{ strip->setPitchShift(originalPitch); });
                restoreIf(strip->getFilterFrequency(), appliedFilterFreq, [&]{ strip->setFilterFrequency(originalFilterFreq); });
                restoreIf(strip->getFilterResonance(), appliedFilterRes, [&]{ strip->setFilterResonance(originalFilterRes); });
                restoreIf(strip->getFilterMorph(), appliedFilterMorph, [&]{ strip->setFilterMorph(originalFilterMorph); });
                if (strip->isFilterEnabled() == appliedFilterEnabled)
                    strip->setFilterEnabled(originalFilterEnabled);
                restoreIf(strip->getDelayMix(), appliedDelayMix, [&]{ strip->setDelayMix(originalDelayMix); });
                restoreIf(strip->getDelayTimeBeats(), appliedDelayTimeBeats, [&]{ strip->setDelayTimeBeats(originalDelayTimeBeats); });
                restoreIf(strip->getDelayFeedback(), appliedDelayFeedback, [&]{ strip->setDelayFeedback(originalDelayFeedback); });
                restoreIf(strip->getDelayLowCutHz(), appliedDelayLowCutHz, [&]{ strip->setDelayLowCutHz(originalDelayLowCutHz); });
                restoreIf(strip->getDelayHighCutHz(), appliedDelayHighCutHz, [&]{ strip->setDelayHighCutHz(originalDelayHighCutHz); });
                restoreIf(strip->getLoopSliceLength(), appliedLoopSliceLength, [&]{ strip->setLoopSliceLength(originalLoopSliceLength); });
                restoreIf(strip->getScratchAmount(), appliedScratchAmount, [&]{ strip->setScratchAmount(originalScratchAmount); });
                restoreIf(strip->getGrainDensity(), appliedGrainDensity, [&]{ strip->setGrainDensity(originalGrainDensity); });
                // The grain render path publishes in-flight smoothed values
                // back into these atomics every sample, so a snapshot compare
                // can never match mid-ramp — additive lanes would ratchet to
                // their rails. These stay on the unconditional restore until
                // the render write-back moves to displayed* atomics.
                ...
                strip->setGrainPositionJitter(originalGrainPositionJitter);
                ...
                strip->setGrainShape(originalGrainShape);

                if (stepSampler != nullptr)
                {
                    restoreIf(stepSampler->getVolume(), appliedStepVol, [&]{ stepSampler->setVolume(originalStepVol); });
                    restoreIf(stepSampler->getPan(), appliedStepPan, [&]{ stepSampler->setPan(originalStepPan); });
                    restoreIf(std::pow(2.0f, static_cast<float>(stepSampler->getPitchOffset()) / 12.0f), appliedStepSpeed,
                              [&]{ stepSampler->setSpeed(originalStepSpeed); });
                    restoreIf(stepSampler->getFilterFrequency(), appliedStepFilterFreq, [&]{ stepSampler->setFilterFrequency(originalStepFilterFreq); });
                    restoreIf(stepSampler->getFilterResonance(), appliedStepFilterRes, [&]{ stepSampler->setFilterResonance(originalStepFilterRes); });
                    if (stepSampler->getFilterType() == appliedStepFilterType)
                        stepSampler->setFilterType(originalStepFilterType);
                    if (stepSampler->isFilterEnabled() == appliedStepFilterEnabled)
    ...
    if (positionInfo.getIsPlaying() && hasPpq && samplesPerBeat > 0.0)
    {
        ...
        auto readModStepValueAtPpq = [&](const ModSequencer& seq, double ppq) -> float
        {
            ...
            const int totalSteps = juce::jmax(ModSteps, ModSteps * lengthBars);
            const int offset = seq.offset.load(std::memory_order_acquire);
            const double stepsPos = ((ppq * 4.0) * static_cast<double>(quantizeModLaneRate(seq.rate.load(std::memory_order_acquire))))
                + static_cast<double>(offset);
            const double wrapped = std::fmod(stepsPos, static_cast<double>(totalSteps));
            const double wrappedPos = (wrapped < 0.0) ? (wrapped + static_cast<double>(totalSteps)) : wrapped;
        ...
        for (int stripIdx = 0; stripIdx < MaxStrips; ++stripIdx)
        {
            ...
            const double division = retriggerDivisionFromAmount(amount);
            if (division <= 0.0)
                continue;
        ...
        if (momentaryStutterActive.load(std::memory_order_acquire) != 0)
        {
            ...
            const double releasePpq = momentaryStutterReleasePpq.load(std::memory_order_acquire);
            const double stutterBlockEndPpq =
                (std::isfinite(releasePpq) && releasePpq >= startPpq)
                    ? juce::jmin(blockEndPpq, releasePpq)
                    : blockEndPpq;
            ...
            if (stutterBlockEndPpq > startPpq)
            {
                for (int stripIdx = 0; stripIdx < MaxStrips; ++stripIdx)
                {
                    const auto idx = static_cast<size_t>(stripIdx);
                    ...
                    while (nextPpq < stutterBlockEndPpq && safety++ < 1024)
                    {
    ...
    while (eventIndex < eventsInBlock.size())
    {
        ...
        while (eventIndex < eventsInBlock.size())
        {
            ...
            // Drop press events cancelled after they were popped (e.g. a Gate
            // release cleared the strip's pending triggers mid-block); the
            // strip would otherwise start and loop with no button held.
            if (!event.isRelease && !quantizeClock.isTriggerCurrent(event))
            {
                ++eventIndex;
                continue;
            }
            ...
            if (auto* strip = getStrip(event.stripIndex))
            {
                if (event.isRelease)
                {
                    if (strip->getPlayMode() == EnhancedAudioStrip::PlayMode::Sample && sampleModeStopCallback)
                    {
                        sampleModeStopCallback(event.stripIndex, false);
                    }
                    ...
                    if (event.clearPendingOnFire)
                        quantizeClock.clearPendingTriggersForStrip(event.stripIndex, false);
                }
                else
                {
...
const EnhancedAudioStrip* ModernAudioEngine::getStrip(int index) const
{
    if (index >= 0 && index < MaxStrips)
        return strips[static_cast<size_t>(index)].get();
    return nullptr;
...
void ModernAudioEngine::setMomentaryStutterReleasePpq(double ppq)
{
    momentaryStutterReleasePpq.store(ppq, std::memory_order_release);
}
...
void ModernAudioEngine::setMacroRetriggerAmount(int stripIndex, float amount01)
{
    if (stripIndex < 0 || stripIndex >= MaxStrips)
        return;

    macroRetriggerAmounts[static_cast<size_t>(stripIndex)].store(
        juce::jlimit(0.0f, 1.0f, amount01),
        std::memory_order_release);
}

float ModernAudioEngine::getMacroRetriggerAmount(int stripIndex) const
{
    if (stripIndex < 0 || stripIndex >= MaxStrips)
        return 0.0f;
    ...
    return juce::jlimit(
        0.0f,
        1.0f,
        macroRetriggerAmounts[static_cast<size_t>(stripIndex)].load(std::memory_order_acquire));
}

void ModernAudioEngine::clearPendingQuantizedTriggersForStrip(int stripIndex, bool bumpGeneration)
{
    if (stripIndex < 0 || stripIndex >= MaxStrips)
        return;
    quantizeClock.clearPendingTriggersForStrip(stripIndex, bumpGeneration);
}
                                                 ...
                                                 int64_t sampleStartSample)
{
    ...
    quantizeClock.scheduleTrigger(stripIndex,
                                  column,
                                  ppq,
                                  strip,
                                  sampleSliceId,
...
void ModernAudioEngine::queueExactTrigger(int stripIndex,
                                          int column,
                                          double targetPPQ,
                                          ...
                                          int64_t sampleStartSample)
{
    ...
    QuantisedTrigger trigger;
    trigger.targetSample = targetSample;
    trigger.targetPPQ = std::isfinite(targetPPQ) ? targetPPQ : lastKnownPPQ.load(std::memory_order_acquire);
    trigger.stripIndex = stripIndex;
    trigger.column = column;
    trigger.sampleSliceId = sampleSliceId;
    trigger.sampleStartSample = sampleStartSample;
    trigger.clearPendingOnFire = false;
    quantizeClock.queueExactTrigger(trigger);
}

void ModernAudioEngine::queueExactRelease(int stripIndex,
                                          int column,
                                          double targetPPQ,
                                          int64_t targetSample,
                                          bool scratchGesture)
{
    ...
    trigger.column = column;
    trigger.clearPendingOnFire = false;
    trigger.isRelease = true;
    trigger.scratchGesture = scratchGesture;
    quantizeClock.queueExactTrigger(trigger);
}

void ModernAudioEngine::triggerStripWithQuantization(int stripIndex,
                                                     int column,
                                                     bool useQuantize,
                                                     int sampleSliceId,
                                                     int64_t sampleStartSample)
{
    ...
    if (useQuantize)
    {
        ...
        scheduleQuantizedTrigger(stripIndex,
                                 column,
                                 0.0,
                                 sampleSliceId,
                                 sampleStartSample);
...
{
    const float clampedMs = juce::jlimit(0.01f, 120.0f, ms);
...
void ModernAudioEngine::setGlobalContinuousTraversal(bool enabled)
{
    for (auto& strip : strips)
    {
        if (strip)
            strip->setContinuousPlayheadTraversal(enabled);
    }
}

bool ModernAudioEngine::usesGlobalContinuousTraversal() const
{
    for (const auto& strip : strips)
    {
        if (strip)
            return strip->usesContinuousPlayheadTraversal();
    }
    return true;
}

void ModernAudioEngine::setGlobalStretchBackend(TimeStretchBackend backend)
{
    const int newValue = static_cast<int>(sanitizeTimeStretchBackend(static_cast<int>(backend)));
...
void ModernAudioEngine::setGlobalTempoMatchBackend(TimeStretchBackend backend)
{
    const int newValue = static_cast<int>(sanitizeTimeStretchBackend(static_cast<int>(backend)));
    if (tempoMatchBackend.load(std::memory_order_acquire) == newValue)
        return;

    tempoMatchBackend.store(newValue, std::memory_order_release);
    for (auto& strip : strips)
    {
        if (strip)
            strip->setTempoMatchBackend(backend);
    }
}

void ModernAudioEngine::setGlobalSoundTouchEnabled(bool enabled)
{
    setGlobalStretchBackend(enabled ? TimeStretchBackend::SoundTouch
                                    : TimeStretchBackend::Resample);
}
        ...
        if (pattern)
        {
            ...
            if (pattern->isPlaying())
            {
                ...
                pattern->processEventsForBeatWindow(previousBeatPos, currentBeatPos,
                    [this, patternIndex](const PatternRecorder::Event& event)
                    {
                        ...
                        else if (event.isNoteOn)
                        {
                            ...
                            triggerStripWithQuantization(event.stripIndex,
                                                         event.column,
                                                         false,
                                                         event.sampleSliceId,
                                                         event.sampleStartSample);
...
int ModernAudioEngine::groupIndexForPattern(int patternIndex)
{
    if (patternIndex < 0 || patternIndex >= MaxPatterns)
        return -1;

    return juce::jlimit(0, MaxGroups - 1, patternIndex);
}

bool ModernAudioEngine::patternRecorderMatchesStrip(int patternIndex, int stripIndex) const
{
    if (stripIndex < 0 || stripIndex >= MaxStrips)
        return false;

    if (patternRecorderIgnoreGroups.load(std::memory_order_acquire) != 0)
        return true;

    const auto* strip = const_cast<ModernAudioEngine*>(this)->getStrip(stripIndex);
    if (strip == nullptr)
        return false;

    return strip->getGroup() == groupIndexForPattern(patternIndex);
}

void ModernAudioEngine::setPatternRecorderIgnoreGroups(bool ignoreGroups)
{
    patternRecorderIgnoreGroups.store(ignoreGroups ? 1 : 0, std::memory_order_release);
}
...
void ModernAudioEngine::startPatternOverdub(int patternIndex)
{
    if (auto* pattern = getPattern(patternIndex))
    {
        if (pattern->isRecording())
        {
            if (pattern->isOverdubbing())
                pattern->extendOverdubRecording();
            return;
        }

        if (pattern->getEventCount() > 0)
            pattern->startOverdub(getTimelineBeat());
        else
            startPatternRecording(patternIndex);
    }
}
...
void EnhancedAudioStrip::setStepPatternLengthSteps(int steps)
{
    ...
    // Steps beyond the new length are KEPT: playback wraps by the stored
    // length so they are inert, and shrinking then re-growing the pattern
    // restores them instead of permanently wiping the upper pages. Explicit
    // pattern-clear operations remain the only destructive path.
...
std::array<bool, 16> EnhancedAudioStrip::getVisibleStepPattern() const
{
    ...
    // Mask to the active pattern length: steps beyond it are retained in the
    // array (non-destructive length changes) but must render as inactive,
    // matching the playback wrap.
    const int totalSteps = juce::jlimit(1, 64, stepPatternLengthSteps.load(std::memory_order_acquire));
    for (int i = 0; i < 16; ++i)
    {
        if (offset + i < totalSteps)
